...
object AssertorSelector {
  ...
  val simple: AssertorSelector = {
    ...
    AssertorSelector(
      ...
      Map(
        "text/html" -> List(/*ValidatorNu.key,*/ HTMLValidator.name, I18nChecker.name, CSSValidator.name),
        "application/xhtml+xml" -> List(/*ValidatorNu.key,*/ HTMLValidator.name, I18nChecker.name, CSSValidator.name),
