...
class ValidationW[E, S](private val inner: Validation[E, S]) {
   ...
   */
  def toImmediateValidation(implicit context: ExecutionContext): FutureValidation[E, S] =
   ...
   */
  def toDelayedValidation(implicit context: ExecutionContext): FutureValidation[E, S] =
    new FutureValidation(Future(inner))
